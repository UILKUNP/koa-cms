...
class TDUrl {
  ...
  appendParam = (key, value, url = window.location.href) => {
    // 向url中添加字段
    let e = {};
    if (!this.typeCheck.isString(key)) {
  ...
  appendParams = (params, url = window.location.href) => {
    // 向url中添加多个字段
    try {
      let e = {};
  ...
  removeParams = (arr, url = window.location.href) => {
    // 从url中去除多个字段
    try {
      let e = {};
...
TDUrl.prototype.typeCheck = (function() {
  // 类型校验
